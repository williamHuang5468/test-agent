 ...
 * @author Reverof
 * usage:
 * start service command>adb shell am start edu.ntut.csie.sslab1321.testagent/edu.ntut.csie.sslab1321.testagent.DummyActivity
 * action command>adb shell am broadcast -a testagent -e action STOP_TESTAGENT
	...
	private boolean openApp(Context context, Bundle bundle) {
		...
		for (ApplicationInfo appInfo : appInfos) {
			if(appInfo.loadLabel(packageManager).equals(appName)) {
				Log.i(TESTAGENT_ACTION, "Installed package: " + appInfo.packageName);
